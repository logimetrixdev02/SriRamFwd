							...
							<table id="dynamic-table" class="table table-striped table-bordered table-hover">
								...
								<tbody>
									...
									<tr id="tr_{{$product_unloading->id}}">
										...
										<td>{{$product_unloading->token_id}}</td>
										<td>{{$product_unloading->product_company_name}}</td>
										<td>{{$product_unloading->wagon_number}}</td>
										<td>{{ $product_unloading->product_name}}</td>
										<td>{{$product_unloading->quantity}}//{{$product_unloading->unit_name}}</td>
										<td>{{$product_unloading->transporter_name}}</td>
										<td>
											@if(!is_null($product_unloading->retailer_id))
											{{$product_unloading->retailer_name}}			
											@endif	
										</td>
										<td>
											@if($product_unloading->loading_slip_type ==1)
