									...
									<div class="col-sm-4 col-sm-offset-4" id="print_area">
										<div class="widget-box transparent" style="border: 1px solid #ccc;">
											...
											<div class="widget-header widget-header-large">
												@php
												$encodedLabourSlipQr = base64_encode('unloading_labour_slip,'.$labour_payment->id);
												@endphp
											...
											<div class="widget-body">
												...
												<div class="widget-toolbar no-border invoice-info" style="float:left;">
													...
													<span class="invoice-info-label" style="margin-top: 5px; font-size: 15px;">{{__('messages.No')}} .#</span>
													<span class="red" style="margin-top: 5px; font-size: 18px;">{{$product_unloading->id}}</span>

													<div class="space"></div><div class="space"></div>
													<span class="invoice-info-label">{{__('messages.Date')}} :</span>
													<span class="blue">{{ date('d/m/Y',strtotime($product_unloading->created_at)) }}</span>
												</div>
												<div class="widget-header widget-header-large">
													@php
													$encodedLoadingSlipQr = base64_encode('unloading_slip,'.$product_unloading->id);
													@endphp
													<h3 class="widget-title grey lighter" style="float:right">
																		...
																		<ul class="list-unstyled spaced">
																			<li>
																				...
																				<div class="col-sm-6"> <span style="float: left;">
																					@if(is_null(!$product_unloading->token))
																					{{getModelById('Token',$product_unloading->token_id)->unique_id}}
																					@endif
																			...
																			<li>
																				<div class="col-sm-5 "><strong style="float: left;font-size: 16px;">  {{__('messages.company')}}</strong></div>
																				<div class="col-sm-6"> <span style="float: left;">
																					@if(isset($product_unloading->master_rake_id))
																					{{getModelById('MasterRake',$product_unloading->master_rake_id)->name}}
																					@endif
																				</span></div><br clear="all">

																			</li>
																			<li>
																				<div class="col-sm-5 "><strong style="float: left;font-size: 16px;"> Warehouse</strong></div>
																				<div class="col-sm-6"> <span style="float: left;">
																					@if(!is_null($product_unloading->warehouse_id))
																					{{getModelById('Warehouse',$product_unloading->warehouse_id)->name}}
																					@endif
																				</span></div><br clear="all">
																			...
																			<li>
																				...
																				<div class="col-sm-6"> <span style="float: left;">{{$product_unloading->product_name}}</span></div><br clear="all">
																			</li>

																			<li>
																				<div class="col-sm-5"><strong style="float: left;font-size: 16px;">{{$product_unloading->unit_name}}</strong></div>
																				<div class="col-sm-6"> <span style="float: left;">{{$product_unloading->quantity}}</span></div><br clear="all">
																			</li>

																			<li>
																				<div class="col-sm-5"><strong style="float: left;font-size: 16px;"> {{__('messages.TruckNumber')}}</strong></div>
																				<div class="col-sm-6"> <span style="float: left;">{{$product_unloading->truck_number}}</span></div><br clear="all">
																			</li>

																			<li>
																				<div class="col-sm-5"><strong style="float: left;font-size: 16px;"> {{__('messages.Transporter')}}</strong></div>
																				<div class="col-sm-6"> <span style="float: left;">{{is_null(getModelById('Transporter',$product_unloading->transporter_id)) ? "":getModelById('Transporter',$product_unloading->transporter_id)->name}}</span></div><br clear="all">
																			</li>

																			<li>
																				<div class="col-sm-5"><strong style="float: left;font-size: 16px;"> {{__('messages.Retailer')}}</strong></div>
																				<div class="col-sm-6"> <span style="float: left;">
																					@if(!is_null($product_unloading->token) && !is_null($product_unloading->retailer_id))
																					{{getModelById('Retailer',$product_unloading->retailer_id)->name}} 
																					<br>
																					({{getModelById('Retailer',$product_unloading->retailer_id)->address}})
																					@endif
																				</span></div><br clear="all">
																			</li>
																			...
																			<li>
																				...
																				<div class="col-sm-6"> <span style="float: left;">
																					@if(is_null($product_unloading->token))
																					{{getModelById('Warehouse',$product_unloading->warehouse_id)->name}}
																					@elseif(!is_null($product_unloading->token) && !is_null($product_unloading->warehouse_id))
																					{{getModelById('Warehouse',$product_unloading->warehouse_id)->name}}
																					@else 
																					{{getModelById('Dealer',$product_unloading->dealer_id)->name}} 
																					<br>
																					({{getModelById('Dealer',$product_unloading->dealer_id)->address1}})
																					@endif
																				</span></div><br clear="all">
																			</li>
