	...
	<div class="main-content-inner">
		<div class="breadcrumbs ace-save-state" id="breadcrumbs">
			<ul class="breadcrumb">
				...
				<li class="active"> Unloading {{__('messages.LabourPayment')}}</li>
			</ul>
		</div>
		...
		<div class="page-content">
			...
			<form action="{{url('/user/pay-warehouse-transfer-unloading-labour')}}" method="post" role="form" id="labourPaymentForm">
				{{ csrf_field() }}
				<div class="container">
					<div class="row">
						<div class="panel panel-primary">
							...
							<div class="panel-body">
								...
								<div id="details" style="display: none;">
									<input type="hidden" name="warehouse_transfer_unloading_id" id="warehouse_transfer_unloading_id">
								...
								<div class="labour-payment-error" style="display: none;">
									<div class="col-md-offset-4">
										<span id="warehouse_unloading_error_span" style="font-size: 30px; color: red; text-align: center;"></span>
									</div>
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	...
	function getLoadingSlipDetails(qr_value){
		var url ="{{url('get-warehouse-transfer-unloading-details')}}";
		if (qr_value == "") {
			swal({
				...
				$("#qr_data").focus();
				$("#warehouse_transfer_unloading_id").val('');
				$("#quantity").val('');
				$("#labour_rate").val('');
				$("#amount_to_pay").val('');
			});
			...
			$.ajax({
				...
				success:function(data){
					...
					if (data.flag) {
						...
						if(data.warehouse_unloading.is_labour_paid == "1"){
							$("#details").html(`<div class="labour-payment-error" style="">
								<div class="col-md-offset-4">
								<span id="warehouse_unloading_error_span" style="font-size: 30px; color: red; text-align: center;">Payment Already Done</span>
								</div>
								</div>`);
							$("#details").show();
						}else{
							...
							$("#qr_data").attr('readonly', true);
							$("#warehouse_transfer_unloading_id").val(data.warehouse_unloading.id);
							$("#labour_name").val(data.warehouse_unloading.labour_name);
							$("#quantity").val(data.warehouse_unloading.quantity);
							$("#labour_rate").val(data.warehouse_unloading.labour_rate);
							$("#amount_to_pay").val(parseFloat(data.warehouse_unloading.labour_rate * data.warehouse_unloading.quantity));
							$("#details").show();
						}
					} else {
						$("#warehouse_unloading_error_span").text(data.message);
