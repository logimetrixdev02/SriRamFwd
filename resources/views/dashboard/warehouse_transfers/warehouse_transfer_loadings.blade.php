...
@section('title','Warehouse Transfer Loadings')
@section('style')
	...
	<div class="main-content-inner">
		<div class="breadcrumbs ace-save-state" id="breadcrumbs">
			<ul class="breadcrumb">
				...
				<li class="active"> Warehouse Transfer Loadings</li>
			</ul>
		</div>

		<div class="page-content">
			<h3 class="header smaller lighter blue"> Warehouse Transfer Loadings</h3>
					...
					<div class="table-responsive">
						...
						<div class="dataTables_borderWrap">
							<table id="dynamic-table" class="table table-striped table-bordered table-hover">
								...
								<tbody>
									@foreach($warehouse_transfer_loadings as $warehouse_transfer_loading)
									<tr>
										
										<td>{{$warehouse_transfer_loading->id}}</td>
										<td>{{$warehouse_transfer_loading->from_warehouse->name}}</td>
										<td>{{$warehouse_transfer_loading->to_warehouse->name}}</td>
										<td>{{$warehouse_transfer_loading->product_brand->name}}</td>
										<td>{{$warehouse_transfer_loading->product->name}}</td>
										<td>{{$warehouse_transfer_loading->quantity}} {{$warehouse_transfer_loading->unit->unit_name}}</td>
										<td>
											@if(!is_null($warehouse_transfer_loading->transporter))
											{{$warehouse_transfer_loading->transporter->name}}
											@else
											-- 
											@endif
										</td>
										<td>{{$warehouse_transfer_loading->truck_number}}</td>
										<td>{{$warehouse_transfer_loading->freight}}</td>
										<td>
										...
										<td>{{$warehouse_transfer_loading->freight_amount_paid}}</td>
										<td>{{$warehouse_transfer_loading->labour}}</td>
										<td>{{$warehouse_transfer_loading->labour_rate}}</td>
										<td>
										...
										<td>{{$warehouse_transfer_loading->labour_paid_amount}}</td>
										<td>{{date('d/m/Y',strtotime($warehouse_transfer_loading->created_at))}}</td>

										<td><a href="{{URL('/user/print-warehouse-transfer-loading-slip/'.$warehouse_transfer_loading->id)}}"><i class="fa fa-print fa-2x"></i></a></td>
