...
<div class="main-content">
	<div class="main-content-inner">
		...
		<div class="page-content">
			...
			<form action="{{url('/user/pay-warehouse-transfer-freight')}}" method="post" role="form" id="freightPaymentForm" onsubmit="return validateForm()">
				{{ csrf_field() }}
				<div class="container">
					<div class="row">
		...
		if (qr_value == "") {
			...
			$.ajax({
				...
				success:function(data){
					...
					$(".loading-bg").hide();
					if (data.flag) {
						if(data.warehouse_loading.is_freight_paid == "1"){
							...
							$("#product_brand").val(data.warehouse_loading.product_brand.name);
							$("#product").val(data.warehouse_loading.product.name);
							$("#transporter").val(data.warehouse_loading.transportername);
							$("#truck_number").val(data.warehouse_loading.truck_number);
							$("#quantity").val(data.warehouse_loading.quantity);
							$("#freight_rate").val(data.warehouse_loading.freight);
							var freight = parseFloat(data.warehouse_loading.freight) * parseInt(data.warehouse_loading.received_quantity);
							$("#freight").val(freight);
							$("#toll_tax").show();
							$("#details").show();
						}
