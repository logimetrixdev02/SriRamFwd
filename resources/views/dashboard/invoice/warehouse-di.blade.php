	...
	<div class="main-content-inner">
		<div class="breadcrumbs ace-save-state" id="breadcrumbs">
			<ul class="breadcrumb">
				...
				<li class="active">Warehouse DIs </li>
			</ul>
		</div>
		...
		<div class="page-content">
			...
			<div class="row">
				<div class="col-xs-12">
					<h3 class="header smaller lighter blue">Warehouse {{__('messages.Invoices')}}</h3>
								...
								<div class="row">
									...
									<div class="col-md-3">
										<div class="form-group">
											...
											<select class="form-control select2" name="type" id="type" onchange="handleChange(this.value)">
												<option value="1" {{isset($type) && $type == 1 ? "selected":""}}> Company to Dealer </option>
												<option value="2" {{isset($type) && $type == 2 ? "selected":""}}> Dealer to Dealer </option>
											</select>

										</div>
									</div>

									<div class="col-md-3" id="from_product_company_div" style="display: {{$type == 1 ? "block":"none"}};">
										<div class="form-group">
									...
									<div class="col-md-3" style="display: {{$type == 2 ? "block":"none"}};" id="from_dealer_div">
										<div class="form-group">
											<label for="from_dealer_id"> From Dealer</label>
											<select class="form-control select2" name="from_dealer_id" id="from_dealer_id">
												<option value=""> Select Dealer</option>
												@foreach($dealers as $dealer)
												<option value="{{$dealer->id}}" {{isset($from_dealer_id) && $from_dealer_id==$dealer->id ? "selected":""}}>{{$dealer->name}}({{$dealer->address1}})</option>
												@endforeach()
											</select>
					...
					<div class="table-header">
						Results for "Warehouse DI" 
						<span class="badge badge-success">{{$total}}</span>
						<div class="widget-toolbar no-border">
							<a class="btn btn-xs bigger btn-danger" href="{{URL('/user/generate-warehouse-di')}}">
						...
						<div class="dataTables_borderWrap">
							<table id="dynamic-table" class="table table-striped table-bordered table-hover">
								...
								<tbody>
									...
									<tr>
										<td>{{$invoice->id}}</td>
										<td>{{getModelById('Warehouse',$invoice->warehouse_id)->name}}</td>
										...
										<td>
											@if(isset($invoice->product_company_id))
											{{getModelById('ProductCompany',$invoice->product_company_id)->name}}
											@else
											{{getModelById('Dealer',$invoice->from_dealer_id)->name}}

											@endif
										</td>
										<td>
											@if(!is_null($invoice->product_brand_id))
											{{getModelById('ProductCompany',$invoice->product_brand_id)->name}}
											@endif
										</td>
										...
										<td>
											...
											<a href="javascript:;" onclick="showPaymentDetails('{{$invoice->payment_amount}}','{{$invoice->payment_date}}','{{$invoice->bank}}','{{$invoice->bank_account_number}}','{{$invoice->bank_reference_number}}','{{$invoice->payment_mode}}')">{{$invoice->total-$invoice->remaining_amount}}</a>
											
										</td>
										<td>{{$invoice->remaining_amount}}</td>
										<td>
											@if(!$invoice->is_paid)
											<a href="javascript:;" onclick="payInvoice({{$invoice->id}},'{{$invoice->remaining_amount}}')"><i class="fa fa-inr fa-2x"></i></a>
											@endif
										</td>
									</tr>
									@endforeach
		...
		<div class="modal fade" id="paymentModal">
			<form action="{{URL('/user/save-warehouse-di-payment')}}" method="post" onsubmit="return validateForm()">
				{{csrf_field()}}
		...
		<script type="text/javascript">
			...
			function handleChange(type){
				if(type == 1){
					...
					$('#from_product_company_div').show();
				}else{
					$('#from_dealer_div').show();
					$('#from_product_company_div').hide();
				}
			}
		</script>
		@endsection
