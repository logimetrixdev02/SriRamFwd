...
<div class="main-content">
	<div class="main-content-inner">
		...
		<div class="page-content">
			...
			<form action="{{url('/user/freight-payment')}}" method="post" role="form" id="freightPaymentForm" onsubmit="return validateForm()">
				{{ csrf_field() }}
				<div class="container">
					<div class="row">
						<div class="panel panel-primary">
							...
							<div class="panel-body">
								...
								<div id="details" style="display: none;">
									<input type="hidden" name="product_loading_id" id="product_loading_id">
									<div class="col-md-4">
										<div class="form-group">
									...
									<div class="col-md-4">
										<div class="form-group">
											<label for="party_name">{{__('messages.Party')}} {{__('messages.Name')}}</label>
											<input type="text" class="form-control" name="party_name" id="party_name" readonly="">
										</div>
									</div>
									<div class="clearfix"></div>
									<div class="col-md-4">
									...
									<div class="col-md-4">
										<div class="form-group">
											...
											<select class="form-control select2" name="destination" id="destination">
												<option value="00"> {{__('messages.Destination')}} {{__('messages.Select')}}</option>
												@foreach($freight_list as $freight)
												<option value="{{$freight->freight}}">{{$freight->destination}} ({{$freight->freight}})</option>
												@endforeach
											</select>
										</div>
									</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		// 	var destination_freight = $("#destination").val();
		// 	var quantity = $("#quantity").val();
		// 	if (destination_freight == "00") {
		// 		swal('Error','Please Select Destination','error');
		// 		$("#freight").val('');
		// 	} else {
		// 		var freight = parseFloat(destination_freight) * parseInt(quantity);
		// 		$("#freight").val(freight);
		// 	}
		// });
	...
	function getLoadingSlipDetails(qr_value){
		var url ="{{url('get-loading-slip-details')}}";
		if (qr_value == "") {
			swal({
			...
			$.ajax({
				...
				success:function(data){
					...
					if (data.flag) {
						...
						$("#product_loading_id").val(data.product_loading.id);
						$("#product_company").val(data.product_loading.product_company_name);
						$("#product").val(data.product_loading.product_name);
						$("#party_name").val(data.product_loading.dealer_name);
						$("#transporter").val(data.product_loading.transporter_name);
						$("#truck_number").val(data.product_loading.truck_number);
						$("#quantity").val(data.product_loading.quantity);
						$("#freight_rate").val(data.product_loading.freight);
						if(data.product_loading.master_rake_id !== null){
							if(data.product_loading.warehouse_id !== null){
								var freight = parseFloat(data.product_loading.freight) * parseInt(data.product_loading.recieved_quantity);
							}else{
								var freight = parseFloat(data.product_loading.freight) * parseInt(data.product_loading.quantity);
						...
						$("#freight").val(freight);
						if(data.product_loading.loading_slip_type == "2"){
							$("#toll_tax").show();
						}
						$("#details").show();
					} else {
