							...
							<table id="dynamic-table" class="table table-striped table-bordered table-hover">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											@if($daily_expense_report->final_approval == 1)
											<span class="label label-success">Approved</span>
											@else
											<span class="label label-warning">Pending</span>
											@endif
										...
										<td>
											<div class="hidden-sm hidden-xs btn-group">
												...
												<a class="btn btn-xs btn-success" href="{{URL('user/rake-payments-detail',$daily_expense_report->id)}}" target="_blank" title="Details">
...
<script>
		...
		if(id == ""){
			...
			function(isConfirm){
				if (isConfirm){
					$('.loading-bg').show();
					$.ajax({
						url: "{{url('/user/approve-rake-payments-report/')}}"+"/"+id,
						type: 'GET',
		...
		if($('#reason').val() == ""){
			...
			function(isConfirm){
				if (isConfirm){
					...
					$.ajax({
						url: "{{url('/user/reject-rake-payments-report/')}}",
						type: 'POST',
	...
	function getRejections(id){
		$('.loading-bg').show();
		$.ajax({
			url: "{{url('/user/rake-payments-report-rejections/')}}"+"/"+id,
			type: 'GET',
			success:function(data){
				$('.loading-bg').hide();
