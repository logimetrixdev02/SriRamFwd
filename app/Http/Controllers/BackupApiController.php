...
class ApiController extends Controller
{
	...
	public function dealers(Request $request){
		...
		if($validator->fails()){
			...
			if($this->checkApiAuth($request)){
				$dealers = \App\Dealer::where('is_active',1)->get();
				if(count($dealers) > 0){
					$response['flag'] 			= true;
	...
	public function product_companies(Request $request){
		$response = array();
		$validator = \Validator::make($request->all(),
			array(
				'api_token' 		=>'required',
		...
		if($validator->fails()){
			...
			if($this->checkApiAuth($request)){
				$product_companies = \App\ProductCompany::where('is_active',1)->get();
				if(count($product_companies) > 0){
					$response['flag'] 			= true;
	...
	public function destinations(Request $request){
		...
		if($validator->fails()){
			...
			if($this->checkApiAuth($request)){
				$destinations = \App\FreightList::all();
				if(count($destinations) > 0){
					$response['flag'] 			= true;
					$response['destinations'] 		= $destinations;
				}else{
					$response['flag'] 			= false;
					$response['message'] 		= "No Destination not found";
				}
			}else{
				$response['flag'] 			= false;
				$response['message'] 		= "सेशन एक्सपायर हो चूका है| कृपया दुबारा लॉगिन करें ";
	...
	public function units(Request $request){
		...
		if($validator->fails()){
			...
			if($this->checkApiAuth($request)){
				$units = \App\Unit::where('is_active',1)->get();
				if(count($units) > 0){
					$response['flag'] 			= true;
					$response['units'] 		= $units;
				}else{
					$response['flag'] 			= false;
					$response['message'] 		= "Units not found";
				}
			}else{
				$response['flag'] 			= false;
				$response['message'] 		= "सेशन एक्सपायर हो चूका है| कृपया दुबारा लॉगिन करें ";
	...
	public function transporters(Request $request){
		...
		if($validator->fails()){
			...
			if($this->checkApiAuth($request)){
				$transporters = \App\Transporter::where('is_active',1)->get();
	...
	public function addTransporter(Request $request){
		...
		if($validator->fails())
		{
			...
			if($transporter->save()){
				...
				$response['transporter_id'] = $transporter->id;
	...
	public function warehouses(Request $request){
		...
		if($validator->fails()){
			...
			if($this->checkApiAuth($request)){
				$warehouses = \App\Warehouse::where('is_active',1)->get();
	...
	public function tokens(Request $request){
		...
		if($validator->fails()){
			...
			if(1){
				...
				$token = \App\Token::with('dealer','master_rake','product','product_company','unit','warehouse')
	...
	public function orders(Request $request){
		...
		if($validator->fails()){
			...
			if(1){
				...
				$orders = \App\Token::with('dealer','master_rake','product','product_company','unit','warehouse')
				->orderBy('id','desc')->get();
